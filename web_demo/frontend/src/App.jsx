...
const App = () => {
  ...
  const addAttribute = async () => {
    ...
    setIsActionLoading(true);
    try {
      await axios.post(`${API_URL}/${activeDb}/${selectedTable}/columns`, {
        name: newColName,
      });
      ...
      toast.error("Backend does not support dynamic columns yet");
    } finally {
      setIsActionLoading(false);
    }
  };
  ...
  const handleCommitUpdate = async () => {
    try {
      ...
      toast.error("Disk write failure");
    }
  };
  ...
  const executeDelete = async () => {
    const { type, target, columnName } = deleteConfirm;
    setIsActionLoading(true);
    try {
      if (type === "database") {
  ...
  const handleShellCommand = async (e) => {
    ...
    try {
      ...
      setTerminalHistory((prev) => [
        ...
        {
          type: "error",
          text: "Instruction failed: No session or syntax error",
        },
      ]);
    }
  };
  ...
  const executeJoin = async () => {
    ...
    try {
      ...
      setTableData(res.data.rows);
      setTableColumns(res.data.columns);
      setSelectedTable(`Relational View`);
      setCurrentView("table-rows");
    } finally {
      setIsLoading(false);
    }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {tables.map((t) => (
                      <Card
                        key={t}
                        onClick={() => fetchRows(t)}
                        className="hover:border-blue-500 cursor-pointer group bg-white relative transition-all shadow-sm"
                      >
                        <Button
                          variant="ghost"
                          size="icon"
                        ...
                        <CardHeader className="flex flex-row items-center gap-4">
                          <div className="bg-slate-50 p-3 rounded-xl group-hover:bg-blue-600 transition-all">
                            <TableIcon
                ...
                <div className="h-[calc(100vh-12rem)] flex flex-col bg-[#020617] rounded-xl overflow-hidden border border-slate-800 shadow-2xl animate-in fade-in duration-500">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 font-mono text-[13px] space-y-1.5 custom-scrollbar selection:bg-blue-500/30 text-slate-300">
                    {terminalHistory.map((line, i) => (
                      <div
                        key={i}
                        className={`whitespace-pre-wrap leading-relaxed ${line.type === "user" ? "text-cyan-400 font-bold" : line.type === "error" ? "text-red-400 font-semibold" : line.type === "success" ? "text-emerald-400 font-medium" : "text-slate-500 italic"}`}
                      >
                        {line.text}
                      </div>
                    ))}
