...
const App = () => {
  ...
  const [terminalHistory, setTerminalHistory] = useState([
    ...
    {
      type: "system",
      text: 'Type "USE <db_name>" or "SELECT FROM <table_name>" to query.',
    },
  ]);
  ...
  const handleShellCommand = async (e) => {
    ...
    try {
      ...
      setTerminalHistory((prev) => [
        ...prev,
        { type: res.data.status, text: res.data.message },
      ]);
      if (
        cmd.toUpperCase().includes("USE") ||
      ...
      setTerminalHistory((prev) => [
        ...prev,
        { type: "error", text: "Engine execution error" },
      ]);
    }
  };
  ...
  const handleInsertRow = async () => {
    if (!activeDb || !selectedTable || Object.keys(rowPayload).length === 0)
      return;
    setIsActionLoading(true);
    try {
      await axios.post(
        `${API_URL}/${activeDb}/${selectedTable}/rows`,
        rowPayload,
      );
      setRowPayload({});
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      setIsActionLoading(false);
      setDeleteConfirm({ show: false, type: null, target: null });
    }
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <AlertDialog
        ...
        onOpenChange={(v) =>
          !v && setDeleteConfirm({ ...deleteConfirm, show: false })
        }
      >
        <AlertDialogContent className="bg-white border-slate-200 shadow-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertCircle className="text-red-500" size={20} /> Confirm
              destruction
            </AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to drop <b>{deleteConfirm.target}</b>? This
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <div className="space-y-1">
              {databases.map((db) => (
                <div key={db} className="flex items-center group px-1">
                  <Button
                    variant="ghost"
                    className={`flex-1 justify-start h-9 text-xs transition-all ${activeDb === db ? "bg-blue-600 text-white" : "text-slate-400 hover:bg-slate-900"}`}
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-9 w-9 text-slate-600 hover:text-red-500 opacity-0 group-hover:opacity-100"
          ...
          {activeDb && (
            <div className="animate-in slide-in-from-left duration-300 border-t border-slate-900 pt-6 space-y-1">
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full px-3 py-2 rounded-md text-sm ${currentView === "db-overview" ? "text-blue-400 bg-slate-900 shadow-inner" : "text-slate-500 hover:bg-slate-900"}`}
              >
                <LayoutGrid size={14} /> Schema Overview
              </button>
              <button
                onClick={() => setCurrentView("relational-explorer")}
                className={`flex items-center gap-3 w-full px-3 py-2 rounded-md text-sm ${currentView === "relational-explorer" ? "text-amber-400 bg-slate-900" : "text-slate-500 hover:bg-slate-900"}`}
              >
                <GitBranch size={14} /> Relational Explorer
              </button>
              <button
                onClick={() => setCurrentView("terminal")}
                className={`flex items-center gap-3 w-full px-3 py-2 rounded-md text-sm ${currentView === "terminal" ? "text-emerald-400 bg-slate-900" : "text-slate-500 hover:bg-slate-900"}`}
              >
                <TerminalIcon size={14} /> Terminal Shell
              </button>

              <div className="pt-4 px-2">
                <p className="text-[10px] uppercase text-slate-500 font-bold mb-3 tracking-widest">
                  Logical Entities
                </p>
                {tables.map((t) => (
                  <button
                    key={t}
                    onClick={() => fetchRows(t)}
                    className={`flex items-center justify-between w-full px-3 py-2 rounded-md text-sm transition-all ${selectedTable === t && currentView === "table-rows" ? "text-white bg-slate-900" : "text-slate-500 hover:bg-slate-900"}`}
                  >
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#f8fafc]">
        <header className="h-16 bg-white border-b flex items-center justify-between px-8 text-slate-600 shadow-sm">
          <div className="flex items-center gap-2 text-sm font-medium">
            <Badge
              variant="outline"
              className="text-blue-600 bg-blue-50 border-blue-200 uppercase font-mono text-[10px]"
            >
              {activeDb ? `ACTIVE: ${activeDb}` : "NO SESSION"}
            </Badge>
            {selectedTable && (
              <>
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => (activeDb ? fetchTables(activeDb) : null)}
            className="gap-2"
          >
          ...
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-400">
              ...
              <p className="font-bold tracking-tight">
                Open a database to establish a logical session
              </p>
            </div>
          ) : (
            <>
                ...
                <div className="space-y-8 animate-in fade-in duration-500">
                  <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 flex items-center gap-1">
                          <Key size={10} className="text-blue-500" /> Primary
                          Key
                        </label>
                        <Input
                          placeholder="e.g. id"
                    ...
                    <Button
                      ...
                      className="mt-6 bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-900/10"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {tables.map((t) => (
                      <Card
                        key={t}
                        onClick={() => fetchRows(t)}
                        className="hover:border-blue-500 cursor-pointer transition-all shadow-sm group bg-white relative"
                      >
                        <Button
                          variant="ghost"
                          size="icon"
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            triggerDelete("table", t);
                          }}
                        >
                          <Trash2 size={14} />
                        </Button>
                        <CardHeader className="flex flex-row items-center gap-4">
                          <div className="bg-slate-50 p-3 rounded-xl group-hover:bg-blue-600 transition-colors">
                            <TableIcon
              ...
              {currentView === "relational-explorer" && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <Card className="p-8 border-blue-200 bg-blue-50/20 shadow-xl shadow-blue-900/5">
                    ...
                    <Button
                      onClick={executeJoin}
                      className="mt-8 bg-blue-600 hover:bg-blue-700 px-10"
                    >
                      Run Inner Join
                    </Button>
                  </Card>
                ...
                <div className="h-[calc(100vh-12rem)] flex flex-col bg-[#020617] rounded-xl overflow-hidden border border-slate-800 shadow-2xl animate-in fade-in duration-500">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 font-mono text-[13px] space-y-1.5 custom-scrollbar selection:bg-blue-500/30">
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      ...
                      <input
                        autoFocus
                        className="flex-1 bg-transparent border-none outline-none text-slate-100 caret-blue-500 font-bold placeholder:text-slate-700"
                        value={commandInput}
                ...
                <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex justify-between items-end">
                    <div className="space-y-1">
                      ...
                      <Badge className="bg-green-100 text-green-700 font-mono text-[9px] uppercase">
                        Integrity Active
                      </Badge>
                    </div>
                      ...
                      <TableHeader className="bg-slate-900">
                        <TableRow>
                          {tableColumns.map((k) => (
                            <TableHead
                              key={k}
                              className="font-bold text-slate-300 uppercase text-[10px] tracking-widest"
                            >
                              {k}
                            </TableHead>
                          ))}
                          <TableHead className="text-right text-slate-300 uppercase text-[10px] tracking-widest">
                            Disk Action
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                        ...
                        {tableData.length === 0 ? (
                          <TableRow>
                            <TableCell
                              ...
                              className="h-40 text-center text-slate-400 italic bg-slate-50/50"
                            >
                              Null set found in logical block
                            </TableCell>
                          </TableRow>
                        ) : (
                          tableData.map((row, i) => (
