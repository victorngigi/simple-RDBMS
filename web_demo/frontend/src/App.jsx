...
const App = () => {
  ...
  const handleShellCommand = async (e) => {
    ...
    try {
      ...
      // Switch context if USE command is successful
      ...
      // Refresh DB list if we created or dropped a database
      if (cmdUpper.includes("DATABASE")) {
        const dbs = await axios.get(`${API_URL}/databases`);
      ...
      setTerminalHistory((prev) => [
        ...
        {
          type: "error",
          text: "Engine error: " + (err.response?.data?.detail || "Unknown"),
        },
      ]);
    }
  };
  ...
  const handleCommitUpdate = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/${activeDb}/${selectedTable}/rows/${editingPk}`,
        rowPayload,
      );
      setEditingPk(null);
      setRowPayload({});
  ...
  const deleteRow = async (row) => {
    ...
    try {
      await axios.delete(
        `${API_URL}/${activeDb}/${selectedTable}/rows/${pkVal}`,
      );
  ...
  const executeJoin = async () => {
    ...
    if (!tableA || !tableB || !colA || !colB) {
      toast.warning("Check join fields");
      return;
    }
    setIsLoading(true);
    try {
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-8">
          ...
          <div>
            ...
            <div className="space-y-1">
              {databases.map((db) => (
                ...
                >
                  <Button
                    variant="ghost"
                    className={`flex-1 justify-start h-9 text-xs transition-all ${activeDb === db ? "bg-blue-600 text-white shadow-lg shadow-blue-900/40" : "text-slate-400 hover:text-white"}`}
            ...
            <div className="mt-4 flex gap-2 px-2">
              ...
              <Button
                size="icon"
                className="h-8 w-10 bg-blue-600 hover:bg-blue-700"
          ...
          <div className="flex items-center gap-2">
            {activeDb && (
              <Button
                variant="ghost"
                size="sm"
                className="text-slate-400 hover:text-red-500 hover:bg-red-50 gap-2 transition-all"
                onClick={() =>
                  triggerDelete(
              ...
              >
                <Trash2 size={14} /> {selectedTable ? "Drop Table" : "Drop DB"}
              </Button>
            )}
            <Button
              variant="outline"
          ...
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-400">
              ...
              <p className="font-bold tracking-tight">
                Establish a logical session via sidebar
              </p>
            </div>
          ) : (
            <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 flex items-center gap-1">
                          <Key size={10} className="text-blue-500" /> PK
                        </label>
                        <Input
                          placeholder="id"
                          value={pkInput}
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-500 uppercase ml-1 flex items-center gap-1">
                          <LinkIcon size={10} className="text-amber-500" /> FK
                        </label>
                        <Input
                          placeholder="col:tbl.col"
                          value={fkInput}
                      ...
                      <Badge
                        className={
                          editingPk
                            ...
                            : "bg-green-100 text-green-700 font-mono text-[9px] uppercase"
                        }
                      >
                        {editingPk
                          ? `MOD_MODE: ${editingPk}`
                    ...
                    <div className="flex flex-wrap gap-2 items-end">
                      {tableColumns.map((col) => (
                        <div key={col} className="space-y-1">
                          ...
                          <Input
                            ...
                            disabled={editingPk && col === pkInput}
                          />
                        </div>
                      ))}
                      <div className="flex gap-1">
                        <Button
                          ...
                          className={editingPk ? "bg-amber-600" : "bg-blue-600"}
                        >
                          {editingPk ? "Update Record" : "Insert"}
                        </Button>
                        {editingPk && (
                          <Button
                            variant="ghost"
                    ...
                    <Table>
                      <TableHeader className="bg-slate-900">
                        <TableRow>
                          ...
                          <TableHead className="text-right text-slate-300 uppercase text-[10px] tracking-widest px-6">
                            Actions
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                ...
                <div className="h-[calc(100vh-12rem)] flex flex-col bg-[#020617] rounded-xl overflow-hidden border border-slate-800 shadow-2xl animate-in fade-in duration-500">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 font-mono text-[13px] space-y-1.5 custom-scrollbar selection:bg-blue-500/30">
                    {terminalHistory.map((line, i) => (
                      <div
                        key={i}
                        className={`whitespace-pre-wrap leading-relaxed ${line.type === "user" ? "text-cyan-400 font-bold" : line.type === "error" ? "text-red-400 font-semibold" : line.type === "success" ? "text-emerald-400 font-medium" : "text-slate-400 italic"}`}
                      >
                        {line.text}
                      </div>
                    ))}
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-800/50 mt-4">
                      ...
                      <span
                        className={`font-bold px-1.5 py-0.5 rounded uppercase text-[10px] tracking-widest border ${activeDb ? "text-cyan-400 bg-cyan-400/10 border-cyan-400/20" : "text-slate-500 bg-slate-500/10 border-slate-500/20"}`}
                      >
                        {activeDb || "system"}
                      </span>
                      <input
