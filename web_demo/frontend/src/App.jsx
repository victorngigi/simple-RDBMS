...
const App = () => {
  ...
  const handleShellCommand = async (e) => {
    ...
    try {
      ...
      setTerminalHistory((prev) => [...prev, { type: "error", text: "Engine error" }]);
    }
  };
          ...
          <div className="flex items-center gap-2">
            {activeDb && (
              <Button 
                ...
                onClick={() => triggerDelete(selectedTable ? "table" : "database", selectedTable || activeDb)}
              >
                ...
                <div className="h-[calc(100vh-12rem)] flex flex-col bg-[#020617] rounded-xl overflow-hidden border border-slate-800 shadow-2xl animate-in fade-in duration-500">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 font-mono text-[13px] space-y-1.5 custom-scrollbar selection:bg-blue-500/30">
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-slate-800/50 mt-4">
                      ...
                      <span className={`font-bold px-1.5 py-0.5 rounded uppercase text-[10px] tracking-widest border ${activeDb ? "text-cyan-400 bg-cyan-400/10 border-cyan-400/20" : "text-slate-500 bg-slate-500/10 border-slate-500/20"}`}>{activeDb || "system"}</span>
                      <input autoFocus className="flex-1 bg-transparent border-none outline-none text-slate-100 caret-emerald-500 font-bold font-mono" value={commandInput} onChange={(e) => setCommandInput(e.target.value)} onKeyDown={handleShellCommand} placeholder="Type HELP for instructions..." />
                    </div>
                  </div>
                </div>
              )}
